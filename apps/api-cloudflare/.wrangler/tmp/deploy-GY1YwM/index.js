...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var HEADERS = /* @__PURE__ */ __name((e) => ({
  "content-type": "application/json",
  "x-appwrite-project": e.appwriteProjectId,
  "x-appwrite-key": e.appwriteApiKey
}), "HEADERS");
var BASE = /* @__PURE__ */ __name((e) => `${e.appwriteEndpoint}/databases/${e.appwriteDatabaseId}`, "BASE");
function buildQueries(queries) {
  return queries.map(([attr, val, type = "string"], i) => {
    const method = type === "number" ? "equal" : "equal";
    const json2 = JSON.stringify({ method, attribute: attr, values: [String(val)] });
    return `queries[${i}]=${encodeURIComponent(json2)}`;
  }).join("&");
}
__name(buildQueries, "buildQueries");
async function dbList(e, collection, queries = []) {
  const qs = queries.length ? buildQueries(queries.map(([a, v]) => [a, v, "string"])) : "";
  const res = await fetch(`${BASE(e)}/collections/${collection}/documents${qs ? "?" + qs : ""}`, { headers: HEADERS(e) });
  return res.json();
}
__name(dbList, "dbList");
async function dbGet(e, collection, docId) {
...
__name(dbGet, "dbGet");
async function dbCreate(e, collection, data) {
...
__name(dbCreate, "dbCreate");
async function dbUpdate(e, collection, docId, data) {
...
async function dbDelete(e, collection, docId) {
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var SESSION_COOKIE_PREFIX = "a_session_";
function json(data, status = 200, extraHeaders) {
  const h = new Headers({ "content-type": "application/json", ...extraHeaders });
  return new Response(JSON.stringify(data), { status, headers: h });
}
__name(json, "json");
function setSessionCookie(resHeaders, projectId, secret, production) {
  resHeaders.set(
    "Set-Cookie",
...
function clearSessionCookie(resHeaders, projectId) {
  resHeaders.set("Set-Cookie", `${SESSION_COOKIE_PREFIX}${projectId}=; Path=/; Max-Age=0; SameSite=Lax`);
}
__name(clearSessionCookie, "clearSessionCookie");
function getSessionToken(req, projectId) {
  const cookieHeader = req.headers.get("cookie") ?? "";
  const match2 = cookieHeader.match(new RegExp(`(?:^|; )${SESSION_COOKIE_PREFIX}${projectId}=(.*?)$`));
...
function extractSecretFromCookie(cookieHeader, projectId) {
  const regex = new RegExp(`a_session_${projectId.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}=(.+?);`);
  const match2 = cookieHeader.match(regex);
  return match2?.[1] ?? "";
}
__name(extractSecretFromCookie, "extractSecretFromCookie");
var auth = new Hono2();
auth.post("/register", async (c) => {
  ...
  try {
    ...
    const sessionSecret = extractSecretFromCookie(cookieHeader, e.appwriteProjectId);
    const setHeaders = new Headers();
    setSessionCookie(setHeaders, e.appwriteProjectId, sessionSecret, e.nodeEnv === "production");
    return c.json({
      success: true,
    ...
    return json({ success: false, data: null, error: { code: "INTERNAL_ERROR", message: err?.message ?? "Registration failed" }, meta: null }, 500);
  }
});
auth.post("/login", async (c) => {
  ...
  try {
    ...
    const setHeaders = new Headers();
    setSessionCookie(setHeaders, e.appwriteProjectId, sessionSecret, e.nodeEnv === "production");
    const q = encodeURIComponent(emailLower);
    const profilesRes = await fetch(
      `${e.appwriteEndpoint}/databases/${e.appwriteDatabaseId}/collections/${e.appwriteCollectionProfiles}/documents?queries[0][attribute]=email0026queries[0][type]=string0026queries[0][value]=${q}`,
      { headers: { "x-appwrite-project": e.appwriteProjectId, "x-appwrite-key": e.appwriteApiKey } }
    );
    const profiles2 = await profilesRes.json();
    return c.json({
      success: true,
...
auth.post("/logout", async (c) => {
  const e = c.get("env");
  const setHeaders = new Headers();
  clearSessionCookie(setHeaders, e.appwriteProjectId);
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  try {
    ...
    const session = await accountRes.json();
    ...
    const profiles2 = await profilesRes.json();
    c.set("user", { id: session.$id, email: session.email, name: session.name, profile: profiles2.documents?.[0] ?? null });
    await next();
  } catch {
...
tasks.get("/", async (c) => {
  const e = c.get("env");
  const r = await dbList(e, e.appwriteCollectionTasks, [["userId", c.get("user").id]]);
...
tasks.post("/", async (c) => {
  ...
  const doc = await dbCreate(e, e.appwriteCollectionTasks, { userId: c.get("user").id, name, createdAt: (/* @__PURE__ */ new Date()).toISOString() });
  return c.json({ success: true, data: doc, error: null, meta: null }, 201);
});
tasks.delete("/:id", async (c) => {
  const e = c.get("env");
  try {
    const doc = await dbGet(e, e.appwriteCollectionTasks, c.req.param("id"));
...
sessions.get("/", async (c) => {
  ...
  const filters = [["userId", c.get("user").id]];
...
sessions.get("/:id", async (c) => {
  const e = c.get("env");
  try {
    const doc = await dbGet(e, e.appwriteCollectionSessions, c.req.param("id"));
...
async function ensureProfile(e, user) {
  ...
  if (!profile3) {
    profile3 = await fetch(`${e.appwriteEndpoint}/databases/${e.appwriteDatabaseId}/collections/${e.appwriteCollectionProfiles}/documents`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-appwrite-project": e.appwriteProjectId, "x-appwrite-key": e.appwriteApiKey },
      body: JSON.stringify({ documentId: ID.unique(), data: { userId: user.id, email: user.email, name: user.name, restRatio: 5, theme: "system", notificationsEnabled: false, soundEnabled: false, createdAt: (/* @__PURE__ */ new Date()).toISOString(), updatedAt: (/* @__PURE__ */ new Date()).toISOString() } })
    }).then((r) => r.json());
  }
  return profile3;
}
...
profiles.get("/", async (c) => {
  const e = c.get("env");
  try {
    const profile3 = await ensureProfile(e, c.get("user"));
...
profiles.patch("/", async (c) => {
  ...
  const updated = await dbUpdate(e, e.appwriteCollectionProfiles, profile3.$id, updates);
...
profiles.post("/avatar", async (c) => {
  ...
  try {
    const updated = await dbUpdate(e, e.appwriteCollectionProfiles, profile3.$id, { avatarUrl, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
    return c.json({ success: true, data: serialize(updated), error: null, meta: null });
  } catch (err) {
...
).route("/api/auth", auth_default).route("/api/tasks", tasks_default).route("/api/sessions", sessions_default).route("/api/analytics", analytics_default).route("/api/me", profiles_default);
