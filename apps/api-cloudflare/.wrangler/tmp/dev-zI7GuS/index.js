...
// .wrangler/tmp/bundle-b8CDpG/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();

// .wrangler/tmp/bundle-b8CDpG/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
...
__name(getEnvFromContext, "getEnvFromContext");
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function getAdminClient(e) {
  return new Client().setEndpoint(e.appwriteEndpoint).setProject(e.appwriteProjectId).setDevKey(e.appwriteApiKey);
}
__name(getAdminClient, "getAdminClient");
function getSessionClient(e, sessionSecret) {
  const c = new Client().setEndpoint(e.appwriteEndpoint).setProject(e.appwriteProjectId);
  if (sessionSecret) c.setSession(sessionSecret);
  return c;
}
__name(getSessionClient, "getSessionClient");
function getDatabases(e) {
  return new Databases(getAdminClient(e));
}
__name(getDatabases, "getDatabases");
async function getProfile(e, userId) {
  const res = await getDatabases(e).listDocuments(
    e.appwriteDatabaseId,
    e.appwriteCollectionProfiles,
...
auth.post("/register", async (c) => {
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      if (/already|exists/i.test(errBody?.message ?? "")) return json({ success: false, data: null, error: { code: "CONFLICT", message: "Email already registered" }, meta: null }, 409);
      throw new Error(errBody?.message ?? "Registration failed");
    }
    const user = await createRes.json();
    const databases = getDatabases(e);
    await databases.createDocument(e.appwriteDatabaseId, e.appwriteCollectionProfiles, ID.unique(), {
      userId: user.$id,
      email: emailLower,
      name,
      restRatio: 5,
    ...
    }).catch((err) => console.error("profile create failed", err?.message));
    ...
    console.error("register failed", e2.message);
    return json({ success: false, data: null, error: { code: "INTERNAL_ERROR", message: e2?.message ?? "Registration failed" }, meta: null }, 500);
  }
});
...
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  try {
    const e = c.get("env");
    const account = new Account(getSessionClient(e, token));
    const session = await account.get();
    let profile3 = null;
    try {
      profile3 = await getProfile(e, session.$id);
    } catch {
      profile3 = null;
    }
...
// src/routes/tasks.ts
var tasks = new Hono2().basePath("/api/tasks").use("*", authMiddleware);
tasks.get("/", async (c) => {
  const e = c.get("env");
  const res = await getDatabases(e).listDocuments(
    e.appwriteDatabaseId,
    e.appwriteCollectionTasks,
...
tasks.post("/", async (c) => {
  ...
  const doc = await getDatabases(e).createDocument(
    e.appwriteDatabaseId,
    e.appwriteCollectionTasks,
    ID.unique(),
...
tasks.delete("/:id", async (c) => {
  const e = c.get("env");
  try {
    const doc = await getDatabases(e).getDocument(e.appwriteDatabaseId, e.appwriteCollectionTasks, c.req.param("id"));
    ...
    await getDatabases(e).deleteDocument(e.appwriteDatabaseId, e.appwriteCollectionTasks, c.req.param("id"));
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var sessions = new Hono2().basePath("/api/sessions").use("*", authMiddleware);
sessions.get("/active", async (c) => {
  const e = c.get("env");
  const res = await getDatabases(e).listDocuments(
    e.appwriteDatabaseId,
    e.appwriteCollectionSessions,
...
sessions.get("/", async (c) => {
  ...
  const res = await getDatabases(e).listDocuments(e.appwriteDatabaseId, e.appwriteCollectionSessions, filters, void 0, true);
  return c.json({
    success: true,
    data: res.documents,
    error: null,
...
sessions.get("/:id", async (c) => {
  const e = c.get("env");
  try {
    const doc = await getDatabases(e).getDocument(e.appwriteDatabaseId, e.appwriteCollectionSessions, c.req.param("id"));
    ...
    const events = await getDatabases(e).listDocuments(
      e.appwriteDatabaseId,
      e.appwriteCollectionEvents,
...
sessions.patch("/:id", async (c) => {
  ...
  let doc;
  try {
    doc = await databases.getDocument(e.appwriteDatabaseId, e.appwriteCollectionSessions, c.req.param("id"));
    if (doc.userId !== c.get("user").id) return c.json({ success: false, data: null, error: { code: "NOT_FOUND", message: "Session not found" }, meta: null }, 404);
  } catch {
    return c.json({ success: false, data: null, error: { code: "NOT_FOUND", message: "Session not found" }, meta: null }, 404);
  }
  ...
  if (b.status === "completed" && b.durationSeconds !== void 0) {
    const profile3 = await getProfile(e, c.get("user").id);
    const ratio = profile3?.restRatio ?? Number(e.restRatioDefault);
    updates.restEarnedSeconds = Math.floor(b.durationSeconds / ratio);
  }
  const updated = await databases.updateDocument(e.appwriteDatabaseId, e.appwriteCollectionSessions, c.req.param("id"), updates);
  if (b.status) {
    await databases.createDocument(
      e.appwriteDatabaseId,
      e.appwriteCollectionEvents,
...
sessions.delete("/:id", async (c) => {
  ...
  try {
    const doc = await getDatabases(e).getDocument(e.appwriteDatabaseId, e.appwriteCollectionSessions, c.req.param("id"));
    if (doc.userId !== c.get("user").id) return c.json({ success: false, data: null, error: { code: "NOT_FOUND", message: "Session not found" }, meta: null }, 404);
    await getDatabases(e).deleteDocument(e.appwriteDatabaseId, e.appwriteCollectionSessions, c.req.param("id"));
    return c.json({ success: true, data: null, error: null, meta: null });
  } catch {
    return c.json({ success: false, data: null, error: { code: "NOT_FOUND", message: "Session not found" }, meta: null }, 404);
  }
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var analytics = new Hono2().basePath("/api/analytics").use("*", authMiddleware);
...
async function listAllCompleted(e, userId) {
  const databases = getDatabases(e);
  const all = [];
  let cursor = 0;
  const limit = 100;
...
async function ensureProfile(e, databases, user) {
  let profile3 = await getProfile(e, user.id);
  if (!profile3) {
    profile3 = await databases.createDocument(e.appwriteDatabaseId, e.appwriteCollectionProfiles, ID.unique(), {
      userId: user.id,
      email: user.email,
      name: user.name,
      restRatio: 5,
...
var profiles = new Hono2().basePath("/api/me").use("*", authMiddleware);
profiles.get("/", async (c) => {
  const e = c.get("env");
  try {
    const profile3 = await ensureProfile(e, getDatabases(e), c.get("user"));
...
profiles.patch("/", async (c) => {
  const e = c.get("env");
  const databases = getDatabases(e);
  const profile3 = await ensureProfile(e, databases, c.get("user"));
  const b = await c.req.json().catch(() => ({}));
  const updates = {};
  ...
  const updated = await databases.updateDocument(e.appwriteDatabaseId, e.appwriteCollectionProfiles, profile3.$id, updates);
...
profiles.post("/avatar", async (c) => {
  ...
  try {
    const updated = await databases.updateDocument(e.appwriteDatabaseId, e.appwriteCollectionProfiles, profile3.$id, { avatarUrl, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
    return c.json({ success: true, data: serialize(updated), error: null, meta: null });
  } catch (err) {
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
var debug3 = new Hono2();
debug3.get("/env", (c) => {
  const e = c.get("env");
  return c.json({
    project_id: e.appwriteProjectId?.slice(0, 8) + "...",
    api_key_len: e.appwriteApiKey?.length ?? 0,
    endpoint: e.appwriteEndpoint,
    db_id: e.appwriteDatabaseId,
    node_env: e.nodeEnv
  });
});
debug3.post("/login-test", async (c) => {
  const e = c.get("env");
  const { email, password } = await c.req.json().catch(() => ({}));
  try {
    const res1 = await fetch(`${e.appwriteEndpoint}/account/sessions/email`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-appwrite-project": e.appwriteProjectId },
    ...
    return c.json({
      login_status: res1.status,
      login_body: text1.slice(0, 300),
      profile_count: profiles2.total,
      profile_ids: profiles2.documents.map((d) => d.$id)
    });
  } catch (err) {
    return c.json({ error: err?.message ?? String(err), stack: err?.stack?.slice(0, 500) }, 500);
  }
});
var debug_default = debug3;
...
var app = new Hono2().use("*", async (c, next) => {
  ...
  origin: "https://flowdoro.email-trisno-sanjaya.pages.dev",
  ...
  (c) => new Response(JSON.stringify({
    success: true,
    data: { status: "ok", service: "flowdoro-api", uptime: process.uptime(), env: c.get("env").nodeEnv },
    error: null,
    meta: null
  }), { headers: { "content-type": "application/json" } })
).route("/api/auth", auth_default).route("/api/tasks", tasks_default).route("/api/sessions", sessions_default).route("/api/analytics", analytics_default).route("/api", profiles_default).route("/debug", debug_default);
var src_default = app;
