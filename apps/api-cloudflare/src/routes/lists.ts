...
lists.get('/', async (c) => {
  const e = c.get('env')
  const r = await dbListAll(e, e.appwriteCollectionLists, [['userId', c.get('user').id]])
...
lists.post('/', async (c) => {
  ...
  // get max sortOrder to set next — use dbListAll to catch >25 lists
  const existing = await dbListAll(e, e.appwriteCollectionLists, [['userId', c.get('user').id]])
...
lists.delete('/:id', async (c) => {
  ...
  try {
    const doc: any = await dbGet(e, e.appwriteCollectionLists, c.req.param('id'))
    if (doc.userId !== c.get('user').id) return c.json({ success: false, data: null, error: { code: 'NOT_FOUND', message: 'List not found' }, meta: null }, 404)
    // un-link tasks from this list (set listId to null) — use dbListAll to catch >100 tasks
    const tasks = await dbListAll(e, e.appwriteCollectionTasks, [['userId', c.get('user').id], ['listId', doc.$id]])
