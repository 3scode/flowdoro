...
<div class="w-full max-w-3xl mx-auto px-4 py-6 pb-20 md:pb-6 flex flex-col gap-4">
  <h1 class="w-full text-2xl font-bold text-balance break-words">History</h1>
  ...
  {:else}
    {#each groupByDate(sessions) as [date, list]}
      <p class="w-full text-xs font-semibold text-text-secondary mt-2 break-words">{date}</p>
      {#each list as s}
        <div class="flex items-center justify-between gap-2 rounded-lg border border-border bg-surface px-4 py-3">
