...
<div class="flex flex-col items-center gap-6 py-8 px-4 max-w-lg mx-auto">
  <h1 class="text-2xl font-bold">Focus</h1>
  {#if state === 'break'}
    <div class="flex flex-col items-center gap-4 py-12">
      ...
      <p class="text-sm text-text-secondary">You focused for {formatDuration(elapsed)}</p>
  ...
  {:else}
    <div class="w-72 h-72 rounded-full border-8 flex flex-col items-center justify-center relative bg-surface" style="border-color: {state === 'running' ? '#0D9488' : '#E2E8F0'}">
      <span class="text-5xl font-mono font-bold tracking-tight" aria-live="polite">{formatTimer(elapsed)}</span>
      <span class="text-sm text-text-secondary mt-1">Focus Time</span>
      {#if state !== 'idle'}<span class="mt-2 text-xs font-medium bg-secondary text-white px-3 py-1 rounded-full">Earned Rest: {formatDuration(earnedRest)}</span>{/if}
    ...
    <div class="w-full rounded-lg border border-border bg-surface px-4 py-3 flex justify-between text-sm">
      <span class="text-text-secondary">Press Space to {state === 'running' ? 'pause' : 'start'}</span>
      <span class="text-text-secondary">S to stop</span>
    </div>
  {/if}
</div>
