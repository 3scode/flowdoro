...
<div class="min-h-screen flex flex-col items-center justify-center px-4 py-12">
  ...
  <p class="text-sm text-text-secondary mb-8">Welcome back</p>
  <form onsubmit={submit} class="w-full max-w-sm rounded-xl border border-border bg-surface p-6 flex flex-col gap-4 shadow-sm">
    ...
    <p class="text-sm text-center text-text-secondary">Don't have an account? <button type="button" class="text-primary font-medium" onclick={() => onNavigate('/register')}>Sign Up</button></p>
  </form>
</div>
